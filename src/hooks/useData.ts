...
import type { Category, SpendingCard, Transaction, CreateCardPayload, CreateTransactionPayload, UpdateTransactionPayload } from '@/types'
...
export function useUpdateCard() {
  ...
  return useMutation({
    mutationFn: async (payload: { id: string } & Partial<CreateCardPayload>) => {
...
export function useDeleteCard() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('spending_cards').delete().eq('id', id)
      if (error) throw error
      return id
    },
    onSuccess: (id) => {
...
export function useSetDefaultVariant() {
  ...
  return useMutation({
    mutationFn: async ({ cardId, variantId }: { cardId: string; variantId: string }) => {
...
export function useCreateTransaction() {
  ...
  return useMutation({
    mutationFn: async (payload: CreateTransactionPayload & { position?: number }) => {
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) throw new Error('Not authenticated')
      ...
      if (payload.source_card_id) {
        await supabase.rpc('increment_card_use_count', { card_id: payload.source_card_id }).throwOnError()
    ...
    onSuccess: (txn) => {
      ...
      qc.invalidateQueries({ queryKey: ['transactions-month', txn.date.slice(0, 7)] })
    },
    onError: (e: Error) => toast.error(e.message),
  })
}
...
export function useUpdateTransaction() {
  ...
  return useMutation({
    mutationFn: async (payload: UpdateTransactionPayload & { date: string }) => {
...
export function useMonthlyTransactions(yearMonth: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      Object.entries(grouped).forEach(([date, txns]) => setTransactionsForDate(date, txns))
