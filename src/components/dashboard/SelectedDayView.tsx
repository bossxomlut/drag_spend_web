...
import { useTransactions, useDeleteTransaction, useSaveTransactionAsCard } from '@/hooks/useData'
...
import {
  ...
  CalendarDays, Pencil, Trash2, BookmarkPlus, GripVertical,
} from 'lucide-react'
...
export function SelectedDayView() {
  ...
  const { setNodeRef, isOver: isOverZone } = useDroppable({ id: `day-${selectedDate}` })
  ...
  useDndMonitor({
    onDragOver({ active, over }) {
      if (!over) { setIsCardOverList(false); return }
      const isCard = active.data.current?.kind === 'card'
      const overTxn = over.data.current?.kind === 'transaction' && over.data.current?.fromDate === selectedDate
      setIsCardOverList(isCard && overTxn)
    },
    onDragEnd() { setIsCardOverList(false) },
    onDragCancel() { setIsCardOverList(false) },
  })
      ...
      <div className="px-5 pt-5 pb-4 border-b border-slate-100">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={goPrev}>
            ...
            <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={goNext}>
              <ChevronRight className="w-4 h-4" />
            </Button>
          </div>

          {!todayFlag && (
            <Button variant="outline" size="sm" className="h-8 text-xs gap-1.5" onClick={goToday}>
          ...
          <div className="flex items-center gap-3 mt-3">
            {totalIncome > 0 && (
              <div className="flex items-center gap-1.5 text-sm">
                ...
                <span className="text-green-600 font-semibold">+{formatCompact(totalIncome)}</span>
              </div>
            )}
            {totalExpense > 0 && (
              <div className="flex items-center gap-1.5 text-sm">
                <TrendingDown className="w-4 h-4 text-red-400" />
                <span className="text-red-500 font-semibold">-{formatCompact(totalExpense)}</span>
              </div>
            )}
            {transactions.length > 0 && (
              <>
                ...
                <span className={cn(
                  'text-sm font-bold',
                  net < 0 ? 'text-red-500' : net > 0 ? 'text-green-600' : 'text-slate-400'
                )}>
                  {net === 0 ? '±0' : `${net > 0 ? '+' : ''}${formatCompact(net)}`}
                </span>
      ...
      {transactions.length > 0 && (
        <div className="border-t border-slate-100 px-5 py-3 bg-slate-50 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-sm">
            ...
            <span className={cn(
              'font-bold',
              net < 0 ? 'text-red-500' : net > 0 ? 'text-green-600' : 'text-slate-500'
            )}>
              {net > 0 ? '+' : ''}{formatVND(net)}
            </span>
          </div>
...
function ConfirmDialog({
  open, onOpenChange, title, description, confirmLabel = 'Xác nhận', danger = false, onConfirm,
}: {
  open: boolean
...
// ─── Transaction Row ───────────────────────────────────────────────────────────
function TransactionItemFull({ transaction: txn }: { transaction: Transaction }) {
  ...
  return (
    <>
      ...
      >
        ...
        {/* Title */}
        <p className="flex-1 min-w-0 text-xs text-slate-700 truncate">{txn.title}</p>
      ...
      <ConfirmDialog
        ...
        confirmLabel="Xóa"
        danger
        onConfirm={() => deleteTransaction.mutate({ id: txn.id, date: txn.date })}
      />

      <EditTransactionDialog open={editOpen} onOpenChange={setEditOpen} transaction={txn} />
    </>
  )
}
