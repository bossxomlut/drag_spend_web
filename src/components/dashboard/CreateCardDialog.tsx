...
export function CreateCardDialog({ open, onOpenChange, editCard }: Props) {
  ...
  function addVariant() {
    setVariants((prev) => [...prev, { label: '', amount: '', is_default: false }])
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{editCard ? 'Chỉnh sửa thẻ' : 'Tạo thẻ mới'}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 pt-1">
          ...
          <div className="space-y-1.5">
            <Label>Tên thẻ <span className="text-red-500">*</span></Label>
            <Input
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label>
                Số tiền
                {variants.length > 1 && (
                  <span className="text-xs text-slate-400 ml-1 font-normal">(chọn mặc định ★)</span>
                )}
              </Label>
              <Button
                type="button"
              ...
              {variants.map((v, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  ...
                  <Input
                    ...
                    value={v.amount}
                    onChange={(e) => updateVariant(idx, 'amount', e.target.value)}
